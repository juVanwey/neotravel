...
- La date du jour est {{ $now.toISO().split('T')[0] }} — utilise cette date comme date_demande ET comme référence pour déduire l'année si le prospect ne la précise pas
- Si le prospect dit "15 juillet" sans préciser l'année, utilise l'année {{ $now.year }} sauf si cette date est déjà passée, auquel cas utilise {{ $now.year + 1 }}
