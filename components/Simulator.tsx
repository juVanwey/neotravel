...
/** Reconstruit le ledger depuis la réponse de /api/devis. */
  ...
  return (
    <div
      style={{
        ...
        margin: "0 auto",
        padding: "clamp(24px,4vw,40px) clamp(16px,4vw,40px) 60px",
      }}
    >
      <div
        style={{
        ...
        >
          <span
            style={{
              ...
              height: 8,
              borderRadius: "50%",
              background: pending ? "var(--warn)" : "var(--ok)",
              animation: "nt-blink 1.4s infinite",
            }}
          />
      ...
      >
        ...
        >
          <div style={{ marginBottom: 26 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
              <label style={{ fontWeight: 800, fontSize: "var(--text-md)", textTransform: "uppercase" }}>
                Passagers
              </label>
              <span className="nt-num" style={{ fontWeight: 900, fontSize: "var(--text-xl)" }}>
                {pax} pers.
              </span>
            </div>
            ...
            <input
              ...
              max={85}
              step={1}
              value={sim.passengers}
              onChange={(e) => patch({ passengers: Number(e.target.value) })}
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            >
              <span>10</span>
              <span>grille ≤180 · puis 2,50/km</span>
              <span>700</span>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 26 }}>
            ...
            <div>
              ...
              <div className="nt-r-sm nt-clip" style={{ display: "flex", border: "2px solid var(--bd)", height: 44 }}>
                <button onClick={() => patch({ roundTrip: false })} style={tripBtn(!sim.roundTrip)}>
                  Simple
                </button>
                <button
                  onClick={() => patch({ roundTrip: true })}
                  style={{ ...tripBtn(sim.roundTrip), borderLeft: "2px solid var(--bd)" }}
                >
                  A/R
                </button>
              </div>
          ...
          >
            ...
            <div style={{ color: "var(--navy)" }}>
              <div style={{ fontWeight: 800, fontSize: "var(--text-base)", textTransform: "uppercase" }}>
                Urgence · {urg.label}
              </div>
          ...
          <div style={{ fontWeight: 800, fontSize: "var(--text-md)", textTransform: "uppercase", marginBottom: 14 }}>
            Options
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "end" }}>
            <NumField
              label="Nuits chauffeur"
              value={sim.driverNights}
              min={0}
              max={20}
              onChange={(v) => patch({ driverNights: v })}
            />

            <button
              onClick={() => patch({ tollsIncluded: !sim.tollsIncluded })}
              className="nt-r-sm"
              style={{
                cursor: "pointer",
                display: "flex",
            ...
            >
              <span style={{ fontSize: "var(--text-xs)", fontWeight: 700, textTransform: "uppercase", color: "var(--ink)" }}>
                Péages
              </span>
              <span
                className="nt-r-sm"
                style={{
                  ...
                  fontWeight: 700,
                  padding: "4px 10px",
                  background: sim.tollsIncluded ? "var(--ok)" : "var(--line)",
                  color: sim.tollsIncluded ? "#fff" : "var(--muted)",
                }}
              >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
            <span style={{ fontSize: "var(--text-2xs)", fontWeight: 600, fontFamily: mono, color: "var(--muted)" }}>
              Guide 80 €/j · nuit 120 €
            </span>
            <button
              onClick={() => patch(DEFAULT_SIM)}
              style={{
                fontSize: "var(--text-xs)",
                fontWeight: 800,
                textTransform: "uppercase",
        ...
        <div style={{ padding: "clamp(22px,3vw,32px)", background: "var(--surface)", position: "relative" }}>
          ...
          >
            <span style={{ fontWeight: 800, fontSize: "var(--text-md)", textTransform: "uppercase" }}>
              Devis détaillé
            </span>
          ...
          {!result && !error && (
            <div style={{ fontFamily: mono, fontSize: "var(--text-sm)", color: "var(--muted)", padding: "20px 0" }}>
              Calcul en cours…
            </div>
          )}
            ...
            <div style={{ opacity: pending ? 0.55 : 1, transition: "opacity .15s" }}>
                  ...
                  {result.options.map((op) => (
                    ...
                    >
                      <div>
                        ...
                        {op.sub && (
                          <div style={{ fontSize: "var(--text-2xs)", fontFamily: mono, color: "var(--muted)" }}>
                            {op.sub}
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ textAlign: "right" }}>
                        ...
                        <div className="nt-num" style={{ fontFamily: mono, fontSize: "var(--text-base)", fontWeight: 700 }}>
                          {op.running}
                        </div>
                      </div>
                    </div>
              ...
              <div className="nt-r-md nt-clip" style={{ marginTop: 16, border: "2px solid var(--bd)", background: "var(--bg)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 15px" }}>
                  ...
                  <span className="nt-num" style={{ fontFamily: mono, fontSize: "var(--text-md)", fontWeight: 700 }}>
                    {result.ht}
                  </span>
                </div>
          ...
          <div className="nt-noprint" style={{ display: "flex", gap: 10, marginTop: 18, flexWrap: "wrap" }}>
            <button className="nt-button nt-button--primary nt-button--lg" style={{ flex: 1, minWidth: 120 }}>
              Valider →
            </button>
            <button onClick={copyRef} className="nt-button nt-button--secondary">
              {copied ? "✓ Copié" : "Copier réf."}
            </button>
            <button onClick={() => window.print()} className="nt-button nt-button--secondary">
              Imprimer
            </button>
          </div>
        </div>
...
function tripBtn(active: boolean): React.CSSProperties {
  return {
    ...
    cursor: "pointer",
    border: "none",
    background: active ? "var(--navy)" : "var(--field)",
